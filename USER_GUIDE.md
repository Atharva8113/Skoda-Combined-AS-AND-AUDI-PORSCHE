...
## Introduction
The **VAG Group Invoice Extractor** is a standalone desktop application designed to automatically extract line-item data from PDF invoices and convert it into a structured Excel (.xlsx) format. It is explicitly built for the logistics and freight forwarding industry to process commercial invoices from Skoda Auto AS, Volkswagen AG, Audi AG, and Audi Hungaria. 
...
* **Combined (All in one superset Excel):** Merges all line items from all selected PDFs into a single, master Excel file. Best for bulk uploading into customs software.
* **Individual (Separate Excel per invoice):** Creates a distinct Excel file for every PDF you selected. Best for keeping records strictly separated by invoice.
...
1. Under "Output Settings", check the **Output Folder** path. By default, it saves the Excel file in the same folder where your PDFs are located.
...
1. Click the blue **Extract & Generate Excel** button in the bottom right corner.
...
4. Open the generated Excel file to review your data.
...
| **Processing Mode (Radio Buttons)** | Determines if output should be 1 Master Excel or Multiple Individual Excels. | Selection |
...
| **Output Folder (Text / Browse)** | The directory where the final Excel(s) will be saved. | Valid Windows File Path |
| **Output Filename (Text Input)** | The name of the file (Only available in *Combined* mode). | Text (no need to type `.xlsx` at the end) |
| **Extract & Generate Excel (Button)** | Starts the PDF reading and data extraction sequence. | N/A |
...
| **Missing Columns in Excel** | If data looks squished, ensure you are opening the `.xlsx` file directly in Excel. This format is native to Excel and should display columns correctly. | Open the file in Excel. |
| **Part Numbers missing leading zeros (e.g., `00010` becomes `10`)** | Excel sometimes tries to convert strings to numbers. | The application forces these columns to "String" type in the Excel file to ensure leading zeros are strictly preserved. |
